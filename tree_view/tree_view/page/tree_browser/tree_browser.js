...
frappe.pages["Tree Browser"].on_page_load = function (wrapper) {
	...
	wrapper.make_tree = function () {
		...
		var ctype = frappe.get_route()[1] || 'Tree';
		return frappe.call({
			method: 'tree_view.tree_view.page.tree_browser.tree_browser.get_children',
			args: {
				ctype: ctype
			},
			callback: function (r) {
...
frappe.pages['Tree Browser'].on_page_show = function (wrapper) {
	// set route
	var ctype = frappe.get_route()[1] || 'Tree';
...
frappe.TreeChart = Class.extend({
	...
	new_node: function () {
		...
		d.set_primary_action(__("Create New"), function () {
			...
			return frappe.call({
				method: 'tree_view.tree_view.page.tree_browser.tree_browser.add_node',
				args: v,
				callback: function (r) {
					if (!r.exc) {
						d.hide();
